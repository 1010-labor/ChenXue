...
void getcann(float *a,int width,int arrayid)//get data from file 输入数据
{
      int i,j;
      float temp;
      FILE *fin;
...
int gcd(int M,int size) //得到最大公因数，用于分块
{
     int i;
     double q;
...
float* initMoveA(float *M,int width,int numpack) //矩阵A的初始移动
{
	float *tempA;
...
float* initMoveB(float *N,int width,int numpack) //矩阵B的初始移动
{
   float *tempB;
...
int main(int argc,char* argv[])
{
  ...
  /*-------------------------------------------------------------------------i-*/       
          ...
          {    
               ...
               for(rn=0;rn<each;rn++)// copy data to other把不连续的数据拷到一起去，再发送
               {
                   for(cn=0;cn<each;cn++)
                   {
