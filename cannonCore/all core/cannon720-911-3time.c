...
int main(int argc,char* argv[])
{
     ...
     if(myid==0)
     {
          ...
          printf("the time of---startComTime---is:%.8fs\n",startComTime);
  ...
  /*-------------------------------------------------------------------------i-*/       
          ...
          {    
               ...
               for(rn=0;rn<each;rn++)// copy data to other
               {
                ...
                timetong=MPI_Wtime();
                printf("the time of---id 0 to id %d send tempM&&tempN---is:%.8fs\n",i,timetong);
...
/*---------------------------------------------------------------------------*/
        if(myid<=numproc)
        {
           int each=width/numpack;
            ...
            timetong=MPI_Wtime();
            printf("the time of---id %d receive tempM&&tempN---is:%.8fs\n",myid,timetong);
        }
    }
/*---------------------------------------------------------------------------*/
if((myid>0)&&(myid<=numproc))
{
    int each=width/numpack;
    timecom=MPI_Wtime();
    printf("id %d --the time of---start 1 compute---is:%.8fs\n",myid,timecom);
    ...
    for(k=0;k<numpack-1;k++)// "K" is suan fa zhong suo yao qiu de bian huan ci shu
    {
        ...
        timetong=MPI_Wtime();
        printf("the time of---id %d to id %d==%d chi start send bufferA---is:%.8fs\n",myid,armR,k,timetong);
        MPI_Send(bufferA,each*each,MPI_FLOAT,armR,armR,MPI_COMM_WORLD);
        timetong=MPI_Wtime();
        printf("the time of---id %d to id %d==%d chi start send bufferB---is:%.8fs\n",myid,armL,k,timetong);
        ...
        timetong=MPI_Wtime();
        printf("the time of---id %d from id %d==%d chi end receive bufferA---is:%.8fs\n",myid,souR,k,timetong);
        MPI_Recv(bufferB,each*each,MPI_FLOAT,souL,myid,MPI_COMM_WORLD,&status);
        timetong=MPI_Wtime();
        printf("the time of---id %d from id %d==%d chi end receive bufferB---is:%.8fs\n",myid,souL,k,timetong);

        timecom=MPI_Wtime();
        printf("id %d --the time of---start %d compute---is:%.8fs\n",myid,k+2,timecom);
        for(i=0;i<each;i++)//ji suan jie guo cun zai ans shu zu zhong
        {  
            for(j=0;j<each;j++)
            {
        ...
        timecom=MPI_Wtime();
        printf("id %d --the time of---end %d compute---is:%.8fs\n",myid,k+2,timecom);
    }
    timetong=MPI_Wtime();
    printf("id %d to id 0--the time of---start send result---is:%.8fs\n",myid,timetong);
    MPI_Send(ans,each*each,MPI_FLOAT,0,3,MPI_COMM_WORLD);
    timetong=MPI_Wtime();
    printf("id %d to id 0--the time of---end send result---is:%.8fs\n",myid,timetong);
    free(bufferA);
    free(bufferB);
    free(ans);
  }
    ...
    if(myid==0)//shou ji jie guo bing shu chu
    {
       ...
       // endComTime=MPI_Wtime();
        ...
        {
             timetong=MPI_Wtime();
             printf("id 0 from id %d--the time of---start receive result---is:%.8fs\n",k,timetong);
             MPI_Recv(ans,each*each,MPI_FLOAT,k,3,MPI_COMM_WORLD,&status);
             timetong=MPI_Wtime();
             printf("id 0 from id %d--the time of---end receive result---is:%.8fs\n",k,timetong);
      ...
      //  free(tempN);
        ...
        printf("total time is:%.8fs\n",totalTime);
        printf("input time is:%.8fs\n",inputTime);
        printf("startComTime is:%.8fs\n",startComTime);
        printf("endComTime is:%.8fs\n",endComTime);
        printf("compute time is:%.8fs\n",comTime);
        printf("output time is:%.8fs\n",outputTime);
   /* else
    {
        MPI_Send(ans,each*each,MPI_FLOAT,0,3,MPI_COMM_WORLD);
    }*/
