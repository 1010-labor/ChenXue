...
// jiang ji suan shi jian da yin chu lai,zhi shi ji suan bu fen de shi jian
#include <stdlib.h>
#include <string.h>
#include <mpi.h>
#include <time.h>
...
int main(int argc,char* argv[])
{
     ...
     double timecom1,timecom2,timetong,timezong=0;
     MPI_Status status;
     int namelen;
...
if((myid>0)&&(myid<=numproc))
{
    int each=width/numpack;
    timecom1=MPI_Wtime();
    printf("id %d --the time of---start 0 compute---is:%.8fs\n",myid,timecom1);
    ...
    timecom2=MPI_Wtime();
    printf("id %d --the time of---end 0 compute---is:%.8fs\n",myid,timecom2);
    timezong+=(timecom2-timecom1);
    ...
    printf("id %d ------the all compute time--is:%.8fs\n",myid,timezong);
