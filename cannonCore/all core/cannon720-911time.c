...
int main(int argc,char* argv[])
{
     ...
     if(myid==0)
     {
          ...
          timetong=MPI_Wtime();
          printf("id 0-----the time of---start Bcast 'width'---is:%.4fs\n",timetong);
          MPI_Bcast(&width,1,MPI_INT,0,MPI_COMM_WORLD);
          timetong=MPI_Wtime();
          printf("id 0-----the time of---end Bcast 'width'--is:%.4fs\n",timetong);
          timetong=MPI_Wtime();
          printf("id 0-----the time of---start Bcast 'numpack'--is:%.4fs\n",timetong);
          MPI_Bcast(&numpack,1,MPI_INT,0,MPI_COMM_WORLD);
          timetong=MPI_Wtime();
          printf("id 0-----the time of---end Bcast 'numpack'--is:%.4fs\n",timetong);
  ...
  /*-------------------------------------------------------------------------i-*/       
          ...
          {    
               ...
               for(rn=0;rn<each;rn++)// copy data to other
               {
                ...
                timetong=MPI_Wtime();
                printf("id 0--to id %d the time of---start first send tempM---is:%.4fs\n",i,timetong);
                MPI_Send(tempM,each*each,MPI_FLOAT,i,5,MPI_COMM_WORLD);
                timetong=MPI_Wtime();
                printf("id 0--to id %d the time of---end first send tempM---is:%.4fs\n",i,timetong);
                timetong=MPI_Wtime();
                printf("id 0--to id %d the time of---start first send tempN---is:%.4fs\n",i,timetong);
                MPI_Send(tempN,each*each,MPI_FLOAT,i,6,MPI_COMM_WORLD);
                timetong=MPI_Wtime();
                printf("id 0--to id %d the time of---end first send tempN---is:%.4fs\n",i,timetong);
          }
           free(tempM);
           free(tempN);
          // startComTime=MPI_Wtime();
    ...
    {
        timetong=MPI_Wtime();
        printf("id %d--the time of---start receive 'width'---is:%.4fs\n",myid,timetong);
        MPI_Bcast(&width,1,MPI_INT,0,MPI_COMM_WORLD);
        timetong=MPI_Wtime();
        printf("id %d--the time of---end receive 'width'---is:%.4fs\n",myid,timetong);
        timetong=MPI_Wtime();
        printf("id %d--the time of---start receive 'numpack'---is:%.4fs\n",i,timetong);
        MPI_Bcast(&numpack,1,MPI_INT,0,MPI_COMM_WORLD);
        timetong=MPI_Wtime();
        printf("id %d--the time of---end receive 'numpack'---is:%.4fs\n",i,timetong);
...
/*---------------------------------------------------------------------------*/
        if(myid<=numproc)
        {
           int each=width/numpack;
            ...
            timetong=MPI_Wtime();
            printf("id %d from id 0--the time of---start receive bufferA---is:%.4fs\n",myid,timetong);
            MPI_Recv(bufferA,each*each,MPI_FLOAT,0,5,MPI_COMM_WORLD,&status);
            timetong=MPI_Wtime();
            printf("id %d from id 0--the time of---end receive bufferA---is:%.4fs\n",myid,timetong);
            timetong=MPI_Wtime();
            printf("id %d from id 0--the time of---start receive bufferB---is:%.4fs\n",myid,timetong);
            MPI_Recv(bufferB,each*each,MPI_FLOAT,0,6,MPI_COMM_WORLD,&status);
            timetong=MPI_Wtime();
            printf("id %d from id 0--the time of---end receive bufferB---is:%.4fs\n",myid,timetong);
        }
    }
/*---------------------------------------------------------------------------*/
if((myid>0)&&(myid<=numproc))
{
    int each=width/numpack;
    timecom=MPI_Wtime();
    printf("id %d --the time of---start first compute---is:%.4fs\n",myid,timecom);
    ...
    timecom=MPI_Wtime();
    printf("id %d --the time of---end first compute---is:%.4fs\n",myid,timecom);
    ...
    for(k=0;k<numpack-1;k++)// "K" is suan fa zhong suo yao qiu de bian huan ci shu
    {
        ...
        timetong=MPI_Wtime();
        printf("id %d to id %d--the time of---%d start send bufferA---is:%.4fs\n",myid,armR,k,timetong);
        MPI_Send(bufferA,each*each,MPI_FLOAT,armR,armR,MPI_COMM_WORLD);
        timetong=MPI_Wtime();
        printf("id %d to id %d--the time of---%d end send bufferA---is:%.4fs\n",myid,armR,k,timetong);
        timetong=MPI_Wtime();
        printf("id %d to id %d--the time of---%d start send bufferB---is:%.4fs\n",myid,armL,k,timetong);
        MPI_Send(bufferB,each*each,MPI_FLOAT,armL,armL,MPI_COMM_WORLD);
        timetong=MPI_Wtime();
        printf("id %d to id %d--the time of---%d end send bufferB---is:%.4fs\n",myid,armL,k,timetong);

        timetong=MPI_Wtime();
        printf("id %d from id %d--the time of---%d start receive bufferA---is:%.4fs\n",myid,souR,k,timetong);
        MPI_Recv(bufferA,each*each,MPI_FLOAT,souR,myid,MPI_COMM_WORLD,&status);
        timetong=MPI_Wtime();
        printf("id %d from id %d--the time of---%d end receive bufferA---is:%.4fs\n",myid,souR,k,timetong);
        timetong=MPI_Wtime();
        printf("id %d from id %d--the time of---%d start receive bufferB---is:%.4fs\n",myid,souL,k,timetong);
        MPI_Recv(bufferB,each*each,MPI_FLOAT,souL,myid,MPI_COMM_WORLD,&status);
        timetong=MPI_Wtime();
        printf("id %d from id %d--the time of---%d end receive bufferB---is:%.4fs\n",myid,souL,k,timetong);

        timecom=MPI_Wtime();
        printf("id %d --the time of---start %d compute---is:%.4fs\n",myid,k+1,timecom);
        for(i=0;i<each;i++)//ji suan jie guo cun zai ans shu zu zhong
        {  
            for(j=0;j<each;j++)
            {
        ...
        timecom=MPI_Wtime();
        printf("id %d --the time of---end %d compute---is:%.4fs\n",myid,k+1,timecom);
    }
    timetong=MPI_Wtime();
    printf("id %d to id 0--the time of---start send result---is:%.4fs\n",myid,timetong);
    MPI_Send(ans,each*each,MPI_FLOAT,0,3,MPI_COMM_WORLD);
    timetong=MPI_Wtime();
    printf("id %d to id 0--the time of---end send result---is:%.4fs\n",myid,timetong);
    free(bufferA);
    free(bufferB);
    free(ans);
  }
    ...
    if(myid==0)//shou ji jie guo bing shu chu
    {
       ...
       // endComTime=MPI_Wtime();
        ...
        {
             timetong=MPI_Wtime();
             printf("id 0 from id %d--the time of---start receive result---is:%.4fs\n",k,timetong);
             MPI_Recv(ans,each*each,MPI_FLOAT,k,3,MPI_COMM_WORLD,&status);
             timetong=MPI_Wtime();
             printf("id 0 from id %d--the time of---end receive result---is:%.4fs\n",k,timetong);
             int R=(k-1)/numpack;
             int L=(k-1)%numpack;
             for(i=0;i<each;i++)
                for(j=0;j<each;j++)
      ...
      //  free(tempN);
        ...
        printf("startComTime is:%.4fs\n",startComTime);
        printf("endComTime is:%.4fs\n",endComTime);
