...
float* initMoveA(float *M,int width,int numpack)
{
  ...
  int i,j,k;
      ...
      /*  printf("the data of tempA before is:\n");
          for(i=0;i<width;i++)
          {
             for(j=0;j<width;j++)
...
if(myid<numproc)
{
    ...
    for(k=0;k<numpack-1;k++)
    {
        ...
        printf("the armR1 dest is %d,myid is %d\n",armR,myid);
        MPI_Send(bufferA,each*each,MPI_FLOAT,armR,armR,MPI_COMM_WORLD);
        printf("the arm2 dest is %d,myid is %d\n",armL,myid);
        ...
        printf("%d---->the zi jin cheng %d receive data M  is\n",k,myid);
           for(i=0;i<each;i++)
           {
               for(j=0;j<each;j++)
                   printf("%f   ",bufferA[i*each+j]);
        ...
        printf("%d----->the zi jin cheng %d receive data N is:\n",k,myid);
           for(i=0;i<each;i++)
           {
                for(j=0;j<each;j++)
                     printf("%f   ",bufferB[i*each+j]);
    ...
    printf("I'm processor %d,my result is:\n",myid);
    for(i=0;i<each;i++)
    { 
        for(j=0;j<each;j++)
          printf("%f   ",ans[i*each+j]);
        printf("\n");
    }
